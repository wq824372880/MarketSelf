...
import com.zeekr.sdk.multidisplay.constant.SdkConstants.MsgDisplayType
...
class MyAppViewModel : BaseViewModel(), InstallStateChangeListener {
            ...
            rxHttpRequest {
                onRequest = {
                    ...
                    withContext(Dispatchers.Default) {
                        //过滤只在前排展示的应用
//                        var activityInfoList = MultiDisplayManager.activityInfoByScreenName(MsgDisplayType.MSG_DISPLAY_INFOTAINMENT_PRIMARY)
                        //过滤不展示应用（不展示应用会设置为强制更新）
                        appPager.list = appPager.list?.filter {
                            it.hideIcon == 0
                        }?.let { ArrayList(it)
                            }
