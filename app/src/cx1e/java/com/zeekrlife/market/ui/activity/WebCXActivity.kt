...
class WebCXActivity : BaseActivity<BaseViewModel, ActivityWebBinding>() {
        ...
        mBind.agentWebView.apply {
            ...
            try {
                if (WebViewFeature.isFeatureSupported(WebViewFeature.FORCE_DARK)) {
                    if (UtilsBridge.isModelNight(this@WebCXActivity.application)) {
                ...
                // 处理方法不存在异常
                Log.e("WebCXActivity", "NoSuchMethodError: ${e.message}")
            } catch (e: NoSuchFieldError) {
                // 处理字段不存在异常
                Log.e("WebCXActivity", "NoSuchFieldError: ${e.message}")
            } catch (e: SecurityException) {
                // 处理安全异常
                Log.e("WebCXActivity", "SecurityException: ${e.message}")
            } catch (e: UnsupportedOperationException) {
                // 处理不支持操作异常
                Log.e("WebCXActivity", "UnsupportedOperationException: ${e.message}")
            } catch (e: Exception) {
                // 处理其他未知异常
                Log.e("WebCXActivity", "Exception: ${e.message}")
            }catch (e: Exception) {
                e.logStackTrace()
            }
