...
import com.zeekrlife.market.ui.viewmodel.SettingCXViewModel
 ...
 * 公共组件库不同，分开写
 */
class SettingCXFragment : BaseFragment<SettingCXViewModel, ActivitySettingsBinding>() {
    ...
    companion object{
        const val TAG = "zzzSettingCXFragment"
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        mBind.settingLayoutAutoUpdate.itemSwitch.apply {
            setSwitchChecked(mViewModel.isAutoUpdate(),true)
        ...
        mBind.settingLayoutAutoUpdate.apply {
            ...
            itemSwitch.setZeekrSwitchListener { checked ->
                ...
                eventViewModel.autoUpdateSwitch.postValue(checked)
            }
        }
    ...
    override fun initObserver() {
        ...
        eventViewModel.autoUpdateSwitch.observe(this){
            LogUtils.e(TAG, "autoUpdateSwitch: $it,isAutoUpdate: ${mViewModel.isAutoUpdate()}")
            mBind.settingLayoutAutoUpdate.itemSwitch.setSwitchChecked(mViewModel.isAutoUpdate(),true)
        }
    }
    ...
    private fun showHomeSettingBadge(show: Boolean) {
        try {
            val act = activity
            if (act is HomeCXActivity && act.mBind.tablayoutBottom.getTabAt(1) != null) {
    ...
    private fun showLoadingDialog() {
        ...
        loadingDialog = ZeekrDialogCreate(requireContext()).show {
            lifecycleOwner(this@SettingCXFragment)
            title("检查更新")
            mergeLayout {
