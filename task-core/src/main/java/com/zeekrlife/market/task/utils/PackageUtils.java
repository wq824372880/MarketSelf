...
public class PackageUtils {
    ...
    public static int getInstallLocation() {
        ...
        if (commandResult.result == 0 && commandResult.successMsg != null && commandResult.successMsg.length() > 0) {
            try {
                ...
                CommExtKt.logStackTrace(e);
                XLog.INSTANCE.e(TAG, "pm get-install-location error");
            }
        }
        return APP_INSTALL_AUTO;
    }
    ...
    /**
     * 检查应用市场内已安装的应用是否是本市场安装的，不是的话改成本市场的包名
     *
     * @param context
     * @param targetPackageName
     * @return
     */
    public static boolean checkInstaller(Context context, String targetPackageName) {
        PackageManager pm = context.getPackageManager();
        try {
            String installerName;
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                InstallSourceInfo installerSourceInfo = pm.getInstallSourceInfo(targetPackageName);
                installerName = installerSourceInfo.getInstallingPackageName();
            } else {
                installerName = pm.getInstallerPackageName(targetPackageName);
            }
            String currentPackageName = context.getPackageName();
            if (installerName == null || !installerName.equals(currentPackageName)) {
                pm.setInstallerPackageName(targetPackageName, currentPackageName);
            }
            return true;
        } catch (Exception e) {
            CommExtKt.logStackTrace(e);
            Log.e("checkInstaller", e.toString());
        }
        return false;
    }
