...
import com.zeekrlife.common.util.constant.MemoryConstants;
import com.zeekrlife.common.util.constant.TimeConstants;
...
public final class ConvertUtils {
    ...
    public static JSONObject bytes2JSONObject(final byte[] bytes) {
        ...
        try {
            return new JSONObject(new String(bytes, Charset.defaultCharset()));
        } catch (Exception e) {
            CommExtKt.logStackTrace(e);
            return null;
        }
    ...
    public static byte[] jsonObject2Bytes(final JSONObject jsonObject) {
        ...
        if (jsonString == null) {
            // 当jsonObject转换为字符串后为null时，返回空字节数组
            return new byte[0];
        }
    ...
    public static JSONArray bytes2JSONArray(final byte[] bytes) {
        ...
        try {
            return new JSONArray(new String(bytes, Charset.defaultCharset()));
        } catch (Exception e) {
            CommExtKt.logStackTrace(e);
            return null;
        }
    ...
    public static byte[] jsonArray2Bytes(final JSONArray jsonArray) {
        ...
        return jsonArray.toString().getBytes(Charset.defaultCharset());
    }
