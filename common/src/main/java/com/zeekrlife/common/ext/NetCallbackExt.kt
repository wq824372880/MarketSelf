    ...
    viewModelScope.launch {
        ...
        runCatching {
            ...
            if (it.code.toString() == BaseNetConstant.EMPTY_CODE) {
                ...
                httpRequestDsl.onEmpty?.invoke(it)
                "操！请求空数据了---->request::${httpRequestDsl.requestCode},---->response:: code::${it.code},message:: ${it.message}".logE()
                ...
                "操！请求出错了---->request::${httpRequestDsl.requestCode},---->response:: code::${it.code},message:: ${it.message}".logE()
            }
            //请求结束时
            if (httpRequestDsl.loadingType != LoadingType.LOADING_NULL) {
...
class HttpRequestDsl {
    ...
    /**
     * 空数据回调，默认为null 如果你传递了他 那么就代表你请求失败的逻辑你自己处理
     */
    var onEmpty: ((Throwable) -> Unit)? = null
